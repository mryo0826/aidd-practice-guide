...
- **人間に打ち返させる ID は、半角英数字で振る。** 選択肢・提案・指摘に付ける ID（`A`／`B`／`F1`／`N6`）は**人間がそのまま入力して裁定に使う**ため、丸数字（①②）のような **IME 変換が要る文字を使わない**。⚠️ **発火の合図: 選択肢を提示するとき・指摘や提案に番号を振るとき。** 最も安い裁定は「AI が提示した記号1文字を人間が返す」形であり、**変換コストの高い文字を選ぶとその設計自体が壊れる**。🔴 **適用範囲は「人間に打ち返させる文」すべて**——**選択肢の裁定・質問・commit／push の承認を含む**（⚠️ **「裁定か承認か」で線を引かない**——**承認だけ ID を外すと、何を答えるのかが読み手の推測になる**）。🔴 **裁定を求めるときは案を2つ以上出し、1つしか無いならその理由を書く**——**1案の提示は、裁定ではなく同意の要求になる**。⚠️ **承認は半角小文字1文字（`y`／`n`）で打ち返せる形にする**——**打ち返しのコストには変換だけでなく修飾キーの打鍵も含まれる**。⚠️ **選択肢をダイアログ UI で出すときも、直前の本文に「ID ＋1行要約」の一覧を置く**——ダイアログだけだと、選択肢以外を言いたいときに内容をコピペする必要があり、**その場で思いついたことを返すコストが跳ね上がる**（本文に一覧があれば、ID 1文字も自由記述も同じコストになる）
