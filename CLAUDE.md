...
- **commit / push は AI が実行し、承認ゲートを人間の検査に使う。** AI は staging 対象とコミットメッセージ案を提示し、**承認を受けてから実行する**。⚠️ **この規約の目的は実行主体を人間にすることではなく、commit の直前に人間の検査を1回挟むこと**——2026-07-27 に、AI が commit を実行しようとした瞬間の承認ゲートで、人間が差分の最終レビューを実際に行った。**ゲートが残っていれば実行主体が AI でも規律は成立し、逆にゲートを外した自動 commit は実行者が誰であれ目的を壊す。** 📌 **「人間がコマンドを打つ」形は既定ではない**（再開手順が壊れていた時期の暫定運用が残骸として残っていたもので、設計意図ではない）。⚠️ **前提条件: 承認ゲートが実際に出ること。** 権限モードによっては commit がゲート無しで通り、**その設定ではこの規約は何も守らないまま守っているように見える**。⇒ **自動承認で運用するなら、AI が commit 前に差分の要約を提示して明示的な承認を求める形へ置き換える**——**検査点は環境が用意してくれるとは限らず、失ったら作り直す。** **`git add -A` は使わない**——未追跡ファイルを無検査で取り込むため（実際にスクリーンショットを混入させた）
