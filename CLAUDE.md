...
- **人間に打ち返させる ID は、半角の英小文字と数字で振る。** 選択肢・提案・指摘に付ける ID（`a`/`b`/`f1`/`n6`）は**人間がそのまま入力して裁定に使う**ため、丸数字（①②）のような **IME 変換が要る文字を使わない**。⚠️ **発火の合図: 選択肢を提示するとき・指摘や提案に番号を振るとき。** 最も安い裁定は「AI が提示した記号1文字を人間が返す」形であり、**変換コストの高い文字を選ぶとその設計自体が壊れる**。🔴 **適用範囲は「人間に打ち返させる文」すべて**——**選択肢の裁定・質問・commit/push の承認を含む**（⚠️ **「裁定か承認か」で線を引かない**——**承認だけ ID を外すと、何を答えるのかが読み手の推測になる**）。🔴 **裁定を求めるときは案を2つ以上出し、1つしか無いならその理由を書く**——**1案の提示は、裁定ではなく同意の要求になる**。⚠️ **英字は小文字で振り、承認は半角小文字1文字（`y`/`n`）で打ち返せる形にする**——**打ち返しのコストには変換だけでなく修飾キー（SHIFT）の打鍵も含まれる**。📌 **小文字の指定が承認の行にしか無く、例示が大文字だった間は、選択肢が大文字で振られ続けた**——**例示は規約として読まれる**。⚠️ **選択肢をダイアログ UI で出すときも、直前の本文に「ID ＋1行要約」の一覧を置く**——ダイアログだけだと、選択肢以外を言いたいときに内容をコピペする必要があり、**その場で思いついたことを返すコストが跳ね上がる**（本文に一覧があれば、ID 1文字も自由記述も同じコストになる）
...
- **人間に操作を依頼するときは、`1` その操作の前に何が挟まるかを1文で書き、`2` 操作する対象の実体（ファイルの中身・画面・コマンドの出力）を先に見せる。** ⚠️ **発火の合図: 人間に手順を渡す文を書く瞬間。** 🔴 **理由: AI は「その画面に至るまでに何を通るか」を知らないまま手順を書ける**——**人間は途中で別の画面に出て、そこで手順が止まる。**
  - 🔴 **同じ軸で反対向きにも壊れる**——**AI の提案文を人間がそのまま指示として返すと、提案に含まれていた誤りごと実行される。** ⇒ ⭐ **裁定すべきは AI の結論ではなく、AI が作った「手順への翻訳」である。**
...
- **区切り候補は AI が申告し、終了・レトロ・続行は人間が裁定する。** `1` タスクが1つ閉じた `2` `docs/design-decisions.md`・`BACKLOG.md`・作業日誌への書き戻しが未了 `3` 文脈が肥大している——が揃ったら申告する（申告は提案であり作業は止めない。詳細は `01_ai-driven-dev-strategy.md` セクション3.6）
  ...
  - 🔴 **利用枠の使用率は、AI も `/usage` で測れる**（Claude Code では `claude -p "/usage"`）。⇒ **AI は枠を測って申告に添え、区切るか・続けるかは人間が裁定する**——⚠️ **AI に見えるのは枠の値だけで、残りの枠を何に使う予定かは見えない。** ⚠️ **枠の値を見た AI が、情報の揃った作業を自分で止めたり範囲を縮めたりしない**（実際に起きた）。📌 **旧記述「利用枠の残りは AI から見えない」は偽だった**（`docs/design-decisions.md`）。📌 **これは分担の変更ではない**（上の行が既に「裁定は人間」と定めている）——**申告に材料を1つ足すだけである。**
...
- **レトロの各項目（Keep / Problem / Try / Action）に一意な ID（`k1`…/`p1`…、Action は `n` 連番）を付ける。** ⚠️ **半角の英小文字と数字で振る**（上の「打ち返させる ID」と同じ理由。人間はレトロの ID で裁定を返す）——**大文字で採番した過去の記録（`K1`・`N16` 等）は書き換えず、`n` 連番は既存の最大番号から続ける。** 採番しない項目は集計から落ち、**レトロの自己申告としての盲点になる**——**過去レトロの前例（precedent）だけの規約は、何も発火させず黙って落ちる**（採番漏れを実際に起こした）
...
2. **Check Action を実行する** — 作業日誌に積まれた前セッションの改善 Action を、作業に着手する前に検証する（やれたか/やれなかったか、結果はどうか）。**これを飛ばすと改善合意が願望のまま蒸発する**（`01_ai-driven-dev-strategy.md` セクション3.6）。人間の記憶に頼らず、この手順として機械的に実行する
