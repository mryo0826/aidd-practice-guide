...
- **人間に操作を依頼するときは、`1` その操作の前に何が挟まるかを1文で書き、`2` 操作する対象の実体（ファイルの中身・画面・コマンドの出力）を先に見せる。** ⚠️ **発火の合図: 人間に手順を渡す文を書く瞬間。** 🔴 **理由: AI は「その画面に至るまでに何を通るか」を知らないまま手順を書ける**——**人間は途中で別の画面に出て、そこで手順が止まる。** ⚠️ **渡す前に「その道具を普段どう使っているか」を1問聞くと、前提のずれが先に出る。**
