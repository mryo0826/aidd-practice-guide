...
- **区切り候補は AI が申告し、終了・レトロ・続行は人間が裁定する。** `1` タスクが1つ閉じた `2` `docs/design-decisions.md`・`BACKLOG.md`・作業日誌への書き戻しが未了 `3` 文脈が肥大している——が揃ったら申告する（申告は提案であり作業は止めない。詳細は `01_ai-driven-dev-strategy.md` セクション3.6）
  ...
  - 🔴 **利用枠の使用率は、AI も自分で測れる**（使用量を表示するコマンドを、AI 側から実行できる場合）。⇒ **AI は枠を測って申告に添え、区切るか・続けるかは人間が裁定する**——⚠️ **AI に見えるのは枠の値だけで、残りの枠を何に使う予定かは見えない。** ⚠️ **枠の値を見た AI が、情報の揃った作業を自分で止めたり範囲を縮めたりしない**（実際に起きた）。📌 **旧記述「利用枠の残りは AI から見えない」は偽だった**（`docs/design-decisions.md`）。📌 **これは分担の変更ではない**（上の行が既に「裁定は人間」と定めている）——**申告に材料を1つ足すだけである。**
