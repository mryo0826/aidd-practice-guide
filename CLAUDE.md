...
- 🔴 **書き戻しを終える前に、「別のマシンで clone / pull して次のセッションを開始した AI」の立場で、自分が書いたものを読み直す。** 検査するのは **`1` 参照先が解決できるか**（ID・リンク・「あのファイル」）**`2` 「次」「現在」「まだ〜していない」が何時点を指すか `3` 件数・範囲を書いた行が実態と合っているか**。⚠️ **発火の合図は2つ**——**書き戻しが終わったと判断した瞬間**と、**push を提案する直前**。
