...
- **公開物（`README.md`・`LICENSE`・`01`〜`06` 本文）を編集したら、版（git tag）の要否を確認する。** `docs/design-decisions.md` §5「バージョンは git tag で表現し…」の表（MAJOR/MINOR/PATCH）に照らし、上げるべきなら申告する。⚠️ **発火の合図はこの1行**——§5 に規則はあるが、公開物を編集した瞬間に思い出す仕組みが無いと黙って落ちる（**発火の合図を持たない規約は落ちる**の実例を 2026-07-24 に踏んだ: README の overclaim 修正で版を上げ忘れかけた）
